...
The `jjbarstats` function is a powerful wrapper around the `ggstatsplot`
package that creates publication-ready bar charts with automatic
statistical testing. This function is designed specifically for
analyzing categorical data relationships in clinical and research
settings.

### Key Features

- **Automatic Statistical Testing**: Performs chi-squared tests,
  Fisher’s exact tests, or other appropriate tests based on your data
- **Multiple Variable Support**: Handle single or multiple dependent
  variables simultaneously
- **Grouped Analysis**: Split analysis by additional grouping variables
- **Flexible Statistical Methods**: Choose from parametric,
  non-parametric, robust, or Bayesian approaches
- **Pairwise Comparisons**: Automatic post-hoc testing with multiple
  comparison correction
- **Professional Visualization**: Publication-ready plots with
...
Let’s start with a basic example using medical study data to compare
treatment responses across different treatment groups:
...
This creates a bar chart showing the distribution of treatment responses
(Complete Response, Partial Response, No Response) across different
treatment groups (Control, Treatment A, Treatment B), along with
...
The `grvar` parameter allows you to split your analysis by an additional
grouping variable, creating separate plots for each level:
...
This creates separate bar charts for male and female patients, allowing
you to examine whether treatment effects differ by gender.
...
When you have more than two groups, pairwise comparisons help identify
...
Different correction methods are available to control for multiple
testing:
...
Analyzing satisfaction levels across different service types and
departments:
...
The function respects jamovi’s theming system when
`originaltheme = FALSE` (default), allowing for consistent styling
across analyses.
...
1.  **Parametric**: Use when data meets assumptions (large sample sizes,
    expected frequencies \>= 5)
2.  **Non-parametric**: Default choice for categorical data, fewer
    assumptions
3.  **Robust**: Good middle ground, less sensitive to outliers
4.  **Bayesian**: When you want to incorporate prior knowledge or report
    Bayes factors
...
3.  **BH (Benjamini-Hochberg)**: Controls false discovery rate, good for
...
When you have empty cells or very small counts, the function
automatically switches to appropriate tests:
...
When you have many categories, consider grouping or using different
visualization:
...
The function automatically handles missing data when `excl = TRUE`
(default):
...
1.  **Chi-squared test**: Tests independence between categorical
    variables
2.  **Effect size (Cramér’s V)**: Measures strength of association (0 =
    no association, 1 = perfect association)
3.  **Confidence intervals**: Provide range of plausible values for the
    effect
...
1.  Describe the statistical test used (chi-squared, Fisher’s exact,
    etc.)
...
The `jjbarstats` function provides a comprehensive solution for
categorical data analysis in clinical and research settings. Its
integration with the `ggstatsplot` ecosystem ensures both statistical
...
The function’s flexibility in statistical methods, multiple comparison
corrections, and visualization options makes it suitable for both
exploratory and confirmatory analysis phases of research projects.
...
- [ClinicoPath package
  documentation](https://clinicopath.github.io/ClinicoPath/)
