...
The `jjhistostats` function provides a comprehensive interface for
creating statistical histograms that visualize the distribution of
continuous variables with normality testing and statistical summaries.
This function is a wrapper around
[`ggstatsplot::gghistostats`](https://www.indrapatil.com/ggstatsplot/reference/gghistostats.html)
and
[`ggstatsplot::grouped_gghistostats`](https://www.indrapatil.com/ggstatsplot/reference/grouped_gghistostats.html),
offering both single and grouped histogram visualizations with advanced
statistical analysis.

## Key Features

- **Multiple distribution assessment methods**: Parametric
  (Shapiro-Wilk), nonparametric (Anderson-Darling), robust, and Bayesian
  approaches
- **Flexible bin width control**: Automatic or manual bin width
  specification
- **Normal curve overlay**: Compare data distribution with theoretical
  normal distribution
- **Centrality indicators**: Display mean, median, or robust centrality
  measures
- **Multi-variable support**: Create multiple histograms in a single
  analysis
...
- **`dep`**: One or more continuous numeric variables for histogram
  creation
...
2.  **“nonparametric”**: Anderson-Darling test
    - More powerful than Shapiro-Wilk for detecting departures from
      normality
...
`.plot2()` methods - No caching infrastructure - Repeated variable
conversion and formula construction - Inefficient multi-variable
handling
...
Redundancy**: Both plot methods now use cached results - **Efficient
Multi-Variable Processing**: Shared data preparation for multiple
histograms - **Better Progress Feedback**: Clear user messaging during
processing
...
- **Reasonable sample size**: At least 10-15 observations per group for
  meaningful analysis
...
- **Use parametric** for well-behaved, moderately-sized samples
  (n=30-5000)
- **Use nonparametric** for larger samples or when concerned about tail
  behavior
...
2.  **Binning issues**
    - If automatic binning creates too few/many bins, use manual
      binwidth
...
3.  **Performance issues**
    ...
    - Multiple variables create multiple plots - expect longer
      processing
...
- **Data exploration**: Understanding variable distributions before
  analysis
- **Assumption checking**: Verifying normality for parametric tests
- **Outlier detection**: Identifying unusual values
- **Data quality assessment**: Detecting measurement errors or data
  entry issues

### Research Scenarios

1.  **Clinical Trials**: Assess baseline characteristics and endpoint
    distributions
2.  **Epidemiology**: Examine exposure and outcome variable
    distributions
3.  **Laboratory Medicine**: Establish reference ranges and quality
    control
4.  **Biomarker Studies**: Understand biomarker distributions across
    populations
...
- **Flexibility**: Comprehensive customization options for binning and
  display
...
data exploration, and any scenario requiring robust distribution
visualization and normality assessment.
