...
Predictive Power Score (PPS) is a modern metric that detects **linear
and non-linear relationships** between variables using machine learning.
Unlike traditional correlation analysis, PPS:
...
The `jpps()` function in ClinicoPath provides a comprehensive interface
...
Let’s start with a simple example showing PPS detecting a non-linear
relationship:
...
This demonstrates how PPS can detect the quadratic relationship between
`x` and `y_quadratic`.
...
Use when you want to analyze **one specific predictor-target
relationship**:
...
Use for **comprehensive relationship exploration** between all
variables:
...
Use to **compare PPS with correlation** and identify where PPS provides
additional insights:
...
**Best for**: Interpretable results, categorical variables, threshold
relationships
...
The jpps function includes intelligent caching to speed up repeated
analyses:
...
**Interpretation**: A high PPS score (\>0.8) suggests treatment dose is
a strong predictor of response rate, likely following a non-linear
dose-response curve typical in pharmacology.
...
The `jpps()` function provides a powerful, modern approach to
...
Remember: PPS is a **“quick and dirty”** exploration tool. Use it to
guide more detailed analysis rather than as a final answer.

------------------------------------------------------------------------

*For more information about ClinicoPath functions, visit our
documentation or explore other vignettes in this series.*
