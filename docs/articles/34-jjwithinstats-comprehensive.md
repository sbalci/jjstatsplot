...
The `jjwithinstats` function is a powerful wrapper around the
[`ggstatsplot::ggwithinstats`](https://www.indrapatil.com/ggstatsplot/reference/ggwithinstats.html)
function that creates publication-ready violin-box plots for
within-subjects (repeated measures) designs. Within-subjects designs are
fundamental in research where the same participants are measured at
multiple time points or under different conditions, making this tool
essential for longitudinal studies, clinical trials, and experimental
research.

### Key Features

- **Repeated Measures Analysis**: Compare 2-4 measurements from the same
  subjects
- **Statistical Flexibility**: Parametric, nonparametric, robust, and
  Bayesian tests
- **Visual Customization**: Violin plots, box plots, individual data
  points, and connection lines
- **Pairwise Comparisons**: Multiple comparison corrections and effect
  size calculations
- **Performance Optimized**: Enhanced caching and data preparation for
  faster rendering
- **Publication-Ready**: High-quality outputs suitable for manuscripts
  and presentations
...
The most common within-subjects design compares measurements before and
...
**Within-Subjects Design Advantages:** - Reduces individual differences
as confounding variables - More statistical power with smaller sample
sizes - Each participant serves as their own control - Ideal for
studying change over time

**Data Requirements:** - Wide format: Each row represents one subject -
Multiple columns for different time points/conditions - Complete cases
(missing data is removed) - Numeric measurements only
...
1.  **Data Preparation Caching**: Long data transformation is cached and
    reused
2.  **Option Preprocessing**: Statistical options and plot arguments are
    processed once
3.  **Hash-based Change Detection**: Only reprocesses when inputs
    actually change
4.  **Efficient Memory Usage**: Minimizes data copying and
    transformation overhead
5.  **Smart Invalidation**: Cache is cleared only when relevant options
    change
...
3.  **Measurement Spacing**: Ensure sufficient time for meaningful
    change
4.  **Data Quality**: Remove extreme outliers that may bias results
5.  **Statistical Choice**: Use nonparametric tests for ordinal data or
    small samples

### Statistical Considerations

1.  **Sphericity**: For parametric tests with \>2 time points, consider
    sphericity assumptions
2.  **Effect Sizes**: Report effect sizes alongside p-values for
    practical significance
3.  **Multiple Comparisons**: Use appropriate corrections for pairwise
    comparisons
...
The `jjwithinstats` function provides a comprehensive solution for
...
**Ideal Applications:** - Pre-post intervention studies - Longitudinal
clinical trials - Educational assessment progressions - Therapy
effectiveness research - Dose-response studies - Recovery time analysis

**Key Advantages:** - Reduces individual differences as confounding
variables - Higher statistical power with smaller samples -
Comprehensive statistical output with effect sizes - Publication-ready
visualizations - Robust error handling and optimization

### Function Reference

For complete parameter documentation, see the ggstatsplot package
documentation: - [CRAN ggstatsplot
documentation](https://cran.r-project.org/web/packages/ggstatsplot/ggstatsplot.pdf) -
[ggwithinstats
function](https://indrajeetpatil.github.io/ggstatsplot/reference/ggwithinstats.html)
